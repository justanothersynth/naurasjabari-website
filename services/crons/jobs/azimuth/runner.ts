import type { OrpcContext } from '@package/types'
import { generateInternalJWT, logError } from '@package/utils'
import { createLocation } from '@service/api/lib/sun-moon'
...
export const { job, subcommand, runJob } = createJobRunner({
  runJob: async ({ jobLogger, schedule }) => {
    ...
    try {
      ...
      await createLocation.internal(orpcContext)(metadata as import('@package/types').SunMoonOrpcInput)
